...
export async function POST(request: Request) {
  try {
    if (!CLEANUP_SECRET) {
      console.error('[pro_upload cleanup] ADMIN_PRO_UPLOAD_CLEANUP_SECRET is not set')
    ...
    if (error) {
      console.error('[pro_upload cleanup] list error:', error)
    ...
    for (const batch of chunk(expired, 100)) {
      ...
      if (removeError) {
        console.error('[pro_upload cleanup] remove error:', removeError, 'paths:', paths)
    ...
    console.error('[pro_upload cleanup] unexpected error:', error)
