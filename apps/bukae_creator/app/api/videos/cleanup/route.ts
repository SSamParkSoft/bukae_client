...
const CLEANUP_SECRET = process.env.ADMIN_VIDEO_CLEANUP_SECRET
const BUCKET = 'videos'
const ROOT_PREFIX = 'videos/jobs'
...
export async function POST(request: Request) {
  try {
    if (!CLEANUP_SECRET) {
      console.error('[videos cleanup] ADMIN_VIDEO_CLEANUP_SECRET is not set')
    ...
    if (error) {
      console.error('[videos cleanup] list error:', error)
    ...
    for (const batch of chunk(expired, 100)) {
      ...
      if (removeError) {
        console.error('[videos cleanup] remove error:', removeError, 'paths:', paths)
    ...
    console.error('[videos cleanup] unexpected error:', error)
