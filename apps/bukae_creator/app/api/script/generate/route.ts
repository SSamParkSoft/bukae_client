...
function generateDummyScenes(
  ...
  // 스타일별 기본 대본 템플릿
  // ConceptType에 맞는 기본 템플릿 매핑
  const getTemplatesForConcept = (concept: ConceptType): string[] => {
    // 각 ConceptType에 맞는 기본 템플릿 반환
    const templateMap: Partial<Record<ConceptType, string[]>> = {
      'HEY_EVERYONE_LOOK': [
        '이거 진짜 미쳤다',
  ...
  // 기본 템플릿 선택 (스타일에 따라)
  const selectedTemplates = getTemplatesForConcept(scriptStyle)
