        ...
        <div className="flex-1 flex gap-4">
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            ...
            <div className="flex flex-col gap-2 mb-2">
              {/* 원래 가격과 할인 배지 */}
              {priceInfo.originalPrice && priceInfo.salePrice && priceInfo.originalPrice > priceInfo.salePrice && (
                <div className="flex items-center gap-2">
                  <span 
                    className="text-[14px] font-medium text-[#a6a6a6] line-through tracking-[-0.14px]"
                    style={{ lineHeight: '19.6px' }}
                  >
                    {priceInfo.originalPrice.toLocaleString()} {priceInfo.currency}
                  </span>
                  {priceInfo.displayDiscount && (
                    <span 
                      className="px-2 py-1 rounded bg-[#dc2626] text-white text-[14px] font-bold tracking-[-0.14px]"
                      style={{ lineHeight: '22.4px' }}
                    >
                      {priceInfo.displayDiscount} 할인
                    </span>
                  )}
                </div>
              )}
