...
- `useStep3Container.ts` - step3 페이지의 모든 비즈니스 로직을 통합하는 컨테이너 훅
  ...
  - 이미지 선택, 드래그 앤 드롭, AI 대본 생성, Extension Storage 이미지 로드 등
...
### 주요 책임
1. **상태 통합**: step3 페이지에 필요한 모든 상태를 통합 관리
2. **이벤트 핸들러 통합**: step3 페이지의 이벤트 핸들러를 통합 제공
3. **Props 제공**: step3 페이지 컴포넌트에 필요한 모든 props 제공
