...
ViewModel 변환  (features/{domain}/hooks/)
   ...
   ▼
Component  (features/{domain}/components/)
   │  값을 받아서 보여주기만 한다
```
...
### `features/{domain}/components/` — UI 컴포넌트
- ViewModel을 props로 받아 렌더링만 한다
- API 호출, 데이터 변환, 포맷팅 로직 금지
- props 타입은 ViewModel 타입을 사용한다
...
### 5. 컴포넌트 작성
```tsx
// features/channelStats/components/ChannelStatsCard.tsx
import type { ChannelStatsViewModel } from '../types/viewModel'
...
import { ChannelStatsCard } from '@/features/channelStats/components/ChannelStatsCard'
